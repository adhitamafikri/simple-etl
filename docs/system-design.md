...
## Technical Flow
1. The *extractors* should retrieve the data from 5 different sheets from the given Google Sheet
    - Admin sheet
    - Member VIP Old Sheet
    - Member VIP V1 Sheet
    - Member VIP V2 Sheet
    - Member VVIP Sheet
2. The *transformers* should transform the data that have been extracted by the *extractors* to the object shape that we've defined:
```typescript
export type User = {
  name: string
  phone: string | null
...
### Constraints
1. The system would use the `email` from the member data to check if the current member already exists in our PostgreSQL
2. The system should insert new member data to the PostgreSQL if it does not exist
3. The system should update the member data in the PostgreSQL if it already exist
